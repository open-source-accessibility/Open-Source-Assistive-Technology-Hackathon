# Appendix F: VS Code Accessibility Reference
...
>
> **Referenced by:** [VS Code Setup & Accessibility Basics](11-vscode-basics.md), [GitHub Copilot](14-github-copilot.md)
...
### Complete Audio Cues List

| Setting | When It Plays | Recommended |
|---------|--------------|-------------|
| `audioCues.clear` | When clearing the terminal or output | `on` |
| `audioCues.chatRequestSent` | When sending a Copilot Chat prompt | `on` |
| `audioCues.chatResponsePending` | While Copilot is generating a response | `auto` |
| `audioCues.chatResponseReceived` | When Copilot finishes responding | `on` |
| `audioCues.debugBreakpoint` | When hitting a breakpoint in debugger | `on` |
| `audioCues.diffLineDeleted` | When navigating over a deleted line in diff | `on` |
| `audioCues.diffLineInserted` | When navigating over an added line in diff | `on` |
| `audioCues.diffLineModified` | When navigating over a modified line in diff | `on` |
| `audioCues.format` | When auto-formatting completes | `auto` |
| `audioCues.lineHasBreakpoint` | When cursor is on a line with a breakpoint | `auto` |
| `audioCues.lineHasError` | When cursor is on a line with an error | `on` |
| `audioCues.lineHasFoldedArea` | When cursor is on a line with collapsed code | `auto` |
| `audioCues.lineHasInlineSuggestion` | When an inline suggestion appears | `on` |
| `audioCues.lineHasWarning` | When cursor is on a line with a warning | `auto` |
| `audioCues.noInlayHints` | When inlay hints are not available | `off` |
| `audioCues.notebookCellCompleted` | When a notebook cell finishes executing | `on` |
| `audioCues.notebookCellFailed` | When a notebook cell fails | `on` |
| `audioCues.onDebugBreak` | When debugger pauses execution | `on` |
| `audioCues.refactorError` | When refactoring fails | `on` |
| `audioCues.save` | When saving a file | `auto` |
| `audioCues.taskCompleted` | When a terminal task completes successfully | `on` |
| `audioCues.taskFailed` | When a terminal task fails | `on` |
| `audioCues.terminalBell` | When terminal bell rings | `on` |
| `audioCues.terminalCommandFailed` | When a terminal command exits with error | `on` |
| `audioCues.terminalQuickFix` | When a terminal quick fix is available | `auto` |
| `audioCues.voiceRecordingStarted` | When voice input begins | `on` |
| `audioCues.voiceRecordingStopped` | When voice input ends | `on` |
| `audioCues.volume` | Volume level (0-100) | `70` |

### Customizing Audio Cue Sounds
...
```json
{
  "audioCues.lineHasError.sound": "file:///C:/Users/YourName/sounds/error.wav",
  "audioCues.taskCompleted.sound": "file:///C:/Users/YourName/sounds/success.wav",
  "audioCues.diffLineInserted.sound": "file:///C:/Users/YourName/sounds/added.wav"
}
```
...
*Back to: [VS Code Setup & Accessibility Basics](11-vscode-basics.md) | [GitHub Copilot](14-github-copilot.md) | [Agent Forge](15-agent-forge.md)*  
